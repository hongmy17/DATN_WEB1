...
.gallery__main {
  ...
  border-radius:var(--r-2xl); overflow:hidden; aspect-ratio:1;
  display:flex; align-items:center; justify-content:center;
  position:relative;
}
.gallery__main-img { font-size:140px; line-height:1; }
.gallery__badge-wrap { position:absolute; top:14px; left:14px; display:flex; flex-direction:column; gap:6px; }
.gallery__wish {
...
.gallery__thumb {
  ...
  border:2px solid var(--border-soft); border-radius:var(--r-lg);
  display:flex; align-items:center; justify-content:center;
  font-size:26px; cursor:pointer;
}
.gallery__thumb:hover, .gallery__thumb.active { border-color:var(--accent); }
.detail__brand { font-size:11px; font-weight:700; text-transform:uppercase; color:var(--accent); margin-bottom:10px; }
.detail__name  { font-family:var(--font-display); font-size:clamp(22px,3vw,34px); font-weight:800; color:var(--ink); margin-bottom:16px; }
.detail__meta  { display:flex; align-items:center; gap:16px; padding:14px 0; border-top:1px solid var(--border-soft); border-bottom:1px solid var(--border-soft); margin-bottom:20px; }
.price-box {
  background:linear-gradient(135deg, var(--accent-light), #fff8f5);
  border:1px solid rgba(200,82,42,.15); border-radius:var(--r-xl);
  padding:20px; margin-bottom:22px;
}
.price-box__current { font-family:var(--font-display); font-size:38px; font-weight:800; color:var(--ink); }
.price-box__old  { font-size:17px; color:var(--ink-muted); text-decoration:line-through; }
.price-box__save { background:var(--red); color:#fff; border-radius:var(--r-full); padding:3px 10px; font-size:12px; }
.option-block { margin-bottom:20px; }
.option-label { font-size:12px; font-weight:700; text-transform:uppercase; color:var(--ink-3); margin-bottom:10px; }
.color-swatches { display:flex; flex-wrap:wrap; gap:12px; }
.color-swatch { width:40px; height:40px; border-radius:50%; cursor:pointer; border:2px solid transparent; }
.color-swatch.active { border-color:var(--ink); transform:scale(1.1); }
.size-chips { display:flex; flex-wrap:wrap; gap:12px; }
.size-chip {
  min-width:60px; padding:10px 16px; border-radius:var(--r-md);
  border:1.5px solid var(--border); font-size:14px; font-weight:600; cursor:pointer; text-align:center;
}
.size-chip.active { background:var(--ink); border-color:var(--ink); color:#fff; }
.add-row { display:flex; gap:10px; margin-top:24px; }
.btn-add-main {
  flex:1; padding:0 24px; height:50px;
...
.guarantees { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; margin-top:22px; }
.guarantee { background:var(--surface); border-radius:var(--r-lg); padding:14px; text-align:center; }
.detail-tabs { margin-top:64px; }
.tab-nav { display:flex; border-bottom:2px solid var(--border-soft); margin-bottom:32px; }
.tab-nav-btn {
...
.qty-ctrl { display:inline-flex; border:1.5px solid var(--border); border-radius:var(--r-md); overflow:hidden; }
.qty-ctrl__btn { width:40px; height:42px; background:var(--surface); font-size:18px; cursor:pointer; border:none; }
.qty-ctrl__input { width:50px; height:42px; text-align:center; border:none; border-left:1.5px solid var(--border); border-right:1.5px solid var(--border); }
@media(max-width:1024px) { .detail-layout { grid-template-columns:1fr; } .gallery { position:static; } }
</style>
@endpush
  ...
  <div class="detail-layout">
    ...
    <div class="gallery">
      <div class="gallery__main">
        <div class="gallery__badge-wrap"><span class="badge badge-hot">Hot 🔥</span></div>
    ...
    <!-- THÔNG TIN -->
    <div>
      <div class="detail__brand">Apple</div>
      ...
      <div class="detail__meta">
        ...
        <div class="detail__meta-sep"></div>
        <div class="detail__meta-item">Đã bán <strong>468</strong></div>
      </div>

      <div class="price-box">
        <div class="price-box__current" id="currentPrice">42.990.000₫</div>
        <div class="price-box__row">
          <span class="price-box__old">49.990.000₫</span>
          <span class="price-box__save">Tiết kiệm 7.000.000₫ (-14%)</span>
        </div>
        <div class="price-box__inst">💳 Trả góp 0% từ <strong>3.582.500₫/tháng</strong> · 12 tháng</div>
      </div>
      ...
      <div class="guarantees">
        <div class="guarantee"><div class="guarantee__icon">🛡️</div><div>Bảo hành 12 tháng</div></div>
        <div class="guarantee"><div class="guarantee__icon">🔄</div><div>Đổi mới 1-1 trong 30 ngày</div></div>
        <div class="guarantee"><div class="guarantee__icon">🚚</div><div>Miễn phí ship toàn quốc</div></div>
      </div>
    </div>
  </div>
  ...
  <div class="detail-tabs">
    ...
    <div class="tab-panel active" id="tab-specs">
      <table class="specs-table">
        <tr><td>CPU / Chip</td><td>Apple M3 Pro (12-core CPU, 18-core GPU)</td></tr>
        <tr><td>RAM</td><td>18GB Unified Memory</td></tr>
        <tr><td>Bộ nhớ trong</td><td>512GB SSD</td></tr>
        <tr><td>Màn hình</td><td>14.2" Liquid Retina XDR, ProMotion 120Hz</td></tr>
        <tr><td>Pin</td><td>70Wh · Lên đến 18 giờ</td></tr>
        <tr><td>Trọng lượng</td><td>1.61 kg</td></tr>
      </table>
    </div>
