...
@section('content')
    <div class="container" style="padding-top:40px">
        ...
        <div class="cart-layout">
            ...
            <div class="summary-section">
                ...
                <a href="{{ route('checkout.index') }}" onclick="return validateCartBeforeCheckout(event)"
                ...
                <a href="{{ route('checkout.index') }}" id="checkoutBtn" class="btn btn-accent btn-full btn-lg"
                    style="gap:8px;margin-top:4px" onclick="return validateCartBeforeCheckout(event)">
    ...
    <div id="clearModal"
        ...
        <div
            style="background:var(--surface,#fff);border-radius:16px;padding:32px 28px;
            max-width:380px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,.18);animation:modalIn .2s ease;">
            <div
                style="width:56px;height:56px;border-radius:50%;background:#fff1f1;
    ...
    <script>
        ...
        /* ── KIỂM TRA TỒN KHO TRƯỚC KHI SANG THANH TOÁN ── */
        async function validateCartBeforeCheckout(event) {
            event.preventDefault();

            const btn = document.getElementById('checkoutBtn');

            if (btn.dataset.checking === '1') {
                Toast.show('Đang cập nhật giỏ hàng, vui lòng thử lại sau giây lát.', 'warning'); // THÊM
                return false;
            }
            // Tránh bấm liên tục trong lúc đang kiểm tra
            if (btn.dataset.checking === '1') return false;

            btn.dataset.checking = '1';

            try {
                // Lấy dữ liệu giỏ hàng mới nhất từ server
                ...
                if (!response.ok || !data.success) {
                    Toast.show(
                        data.message || 'Không thể kiểm tra tồn kho. Vui lòng thử lại.',
                        'error'
                    );
                    return false;
                }

                const invalidItem = data.data.find(item => {
                    if (!item.manage_stock) {
                        return false;
                    }

                    return Number(item.qty) > Number(item.stock);
                });

                // Có sản phẩm vượt tồn kho → DỪNG TẠI TRANG GIỎ HÀNG
                if (invalidItem) {
                    Toast.show(
                        `"${invalidItem.name}" chỉ còn ${invalidItem.stock} sản phẩm trong kho, bạn đang chọn ${invalidItem.qty}.`,
                        'error'
                    );
                ...
                // Tất cả sản phẩm đều hợp lệ → mới chuyển sang thanh toán
