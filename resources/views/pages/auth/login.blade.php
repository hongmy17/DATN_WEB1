...
<div class="auth-wrap">
  <div class="auth-card">
    ...
    <div class="auth-panel active" id="loginPanel">
      <div class="social-btn" onclick="Toast.show('Đăng nhập Google sắp ra mắt!','info')">
        <span>🇬</span> Tiếp tục với Google
      </div>
      <div class="social-btn" onclick="Toast.show('Đăng nhập Facebook sắp ra mắt!','info')">
      ...
      <div class="form-group">
        ...
        <div class="input-icon"><span class="input-icon__icon">📧</span><input type="text" class="form-control" placeholder="email@gmail.com hoặc 0901234567" id="loginId"></div>
      </div>
      <div class="form-group">
        <label class="form-label">Mật khẩu</label>
        <div class="input-icon"><span class="input-icon__icon">🔒</span><input type="password" class="form-control" placeholder="••••••••" id="loginPw"></div>
      </div>
      <div class="d-flex justify-between align-center mb-16">
        <label class="checkbox-wrap"><input type="checkbox"><span>Ghi nhớ đăng nhập</span></label>
        <a href="{{ url('quen-mat-khau') }}" style="font-size:13px;color:var(--accent)">Quên mật khẩu?</a>
      </div>
      <button class="btn btn-primary btn-full btn-lg" onclick="doLogin()">Đăng Nhập</button>
      <p class="text-center mt-16" style="font-size:13px;color:var(--ink-muted)">Chưa có tài khoản? <a onclick="switchAuth('register',document.querySelectorAll('.auth-tab')[1])" style="color:var(--accent);cursor:pointer;font-weight:600">Đăng ký ngay</a></p>
    </div>
...
<script>
  ...
  function doLogin() {
    ...
    if (!id || !pw) { Toast.show('Vui lòng nhập đầy đủ thông tin', 'error'); return; }
  ...
  function doRegister() {
    Toast.show('Đăng ký thành công! Kiểm tra email xác nhận 📧', 'success');
    setTimeout(() => switchAuth('login', document.querySelectorAll('.auth-tab')[0]), 1200);
  }
