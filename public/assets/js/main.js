...
const Cart = {
    get() {
        try { return JSON.parse(localStorage.getItem('nx_cart') || '[]'); }
        catch { return []; }
    },
    _save(c) {
    ...
    _idx(variantId) {
        return Cart.get().findIndex(i => String(i.variant_id) === String(variantId));
    },
    ...
    async add(item) {
        ...
        if (window.__authUser) {
            const res = await cartApi('POST', '', { variant_id: item.variant_id, quantity: qty });
            if (!res.success) { Toast.show(res.message ?? 'Thêm vào giỏ thất bại', 'error'); return; }
            const c = Cart.get();
            ...
            if (idx >= 0) {
                ...
                c.push({ ...item, qty: res.data.qty, cart_item_id: res.data.cart_item_id });
            }
            Cart._save(c);
        } else {
            const c = Cart.get();
    ...
    async updateQty(variantId, qty) {
        if (qty <= 0) { await Cart.removeByVariantId(variantId); return; }
        const c = Cart.get();
        const idx = Cart._idx(variantId);
        if (idx < 0) return;
    ...
    async syncToServer() {
        ...
        if (!local.length) { await Cart.loadFromServer(); return; }
        const res = await cartApi('POST', '/sync', {
            items: local.map(i => ({ variant_id: i.variant_id, quantity: i.qty }))
        });
...
const Wishlist = {
    get() {
        try { return JSON.parse(localStorage.getItem('nx_wish') || '[]'); }
        catch { return []; }
    },
    ...
    toggle(id, name, price, img, slug) {
        ...
        if (idx >= 0) { w.splice(idx, 1); Toast.show('Đã xoá khỏi yêu thích', 'info'); }
        else { w.push({ id: sid, name: name || 'Sản phẩm', price: price || 0, img: img || '', slug: slug || '' }); Toast.show('Đã thêm vào yêu thích', 'success'); }
        Wishlist.save(w);
    },
    has(id) { return Wishlist.get().some(i => String(i.id) === String(id)); },
...
function initWishlistButtons() {
    document.addEventListener('click', function(e) {
        ...
        Wishlist.toggle(btn.dataset.wishId, btn.dataset.wishName || 'Sản phẩm',
            parseInt(btn.dataset.wishPrice || '0'), btn.dataset.wishImg || '', btn.dataset.wishSlug || '');
    });
}
